...
🌐 **Live:** https://childcompass.vercel.app
...
**ChildCompass** is a childcare and kids activity marketplace for parents in Erfurt, Germany. Children ages 0–6. Parents discover, filter, and book trusted activities. Providers publish listings and manage booking requests. Admins seed provider data via AI-powered scraping tools.

Built entirely through conversational AI development with Claude Code using a **3 sub-agent architecture** — DB Agent (`db` branch), Backend Agent (`backend` branch), and Frontend Agent (`frontend` branch) — each with strict file ownership boundaries defined in `.claude/rules/`.
...
| Connection Pooling | PgBouncer (via Supabase Transaction Pooler) | — |
| Email | Resend | — |
| Admin Scraper | Anthropic Claude API + built-in `web_search` tool | — |
...
│   │   ├── email.ts                      # Resend email helpers (booking notifications)
...
| Feature | Status |
|---|---|
| Activity discovery + Mapbox map | ✅ Live |
