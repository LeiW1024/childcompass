#!/usr/bin/env node
// scripts/update-activities.mjs
// Fetch fresh childcare/activity data for Erfurt, upsert to DB, geocode locations.
// Usage:
//   node --env-file=.env.local scripts/update-activities.mjs
//   node --env-file=.env.local scripts/update-activities.mjs SPORTS
...
async function upsertListing(provider, raw) {
  ...
  const listing = await prisma.listing.create({
    data: {
      ...
      description:   raw.description || "Weitere Informationen folgen.",
...
async function main() {
  ...
  if (arg && !ALL_CATEGORIES.includes(arg)) {
